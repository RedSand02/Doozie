...
export interface ITests extends IProps {
}
...
function TabPanel(props: ITabPanel) {
    ...
    return (
        <Typography
            ...
            role="tabpanel"
            hidden={value !== index}
            id={`full-width-tabpanel-Tests-${index}`}
            aria-labelledby={`full-width-tab-Tests-${index}`}
            {...other}
        >
...
function a11yProps(index: number) {
    return {
        id: `full-width-tab-Tests-${index}`,
        'aria-controls': `full-width-tabpanel-Tests-${index}`,
    };
}
...
export default function Tests(props: ITests) {
